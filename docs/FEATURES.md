...
![diagram (4).jpg](attachment:2ba6dc75-a76a-4d61-8747-0f88051827de:diagram_(4).jpg)
...
- 스캔 확인 모달:
  - 인식된 방문자 정보 미리보기
  - "확인" 버튼: 방문자 리스트에 추가 후 Firestore 전송
  - "수정" 버튼: 방문자 리스트로 추가하여 수동 편집 가능
  - "닫기" 버튼: 스캔 결과 취소
...
- useMemo로 그룹화 성능 최적화
- localStorage에 임시 저장 (일일 통계용)
- Firestore 전송 성공 시 리스트 초기화
...
**갱신 주기**: localStorage는 자정 초기화 (과거 날짜 데이터 자동 삭제)
